...
import { AdminStageBadge } from "./AdminStageBadge";
import { ALL_STAGES, formatDate, timeAgo } from "./adminDashboardHelpers";
...
function PipelineCard({ item, onView }: { item: VerificationItem; onView: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <AdminStageBadge stage={item.stage} />
      </div>
      ...
      {isRejected ? (
        ...
        <div className="flex flex-col">
          {ALL_STAGES.map((stage, i) => {
            ...
            const dotColor = isCompleted ? "#34d399" : isCurrent ? "#818cf8" : "rgba(255,255,255,0.1)";
            const lineColor = isCompleted ? "rgba(52,211,153,0.3)" : "rgba(255,255,255,0.06)";
            ...
            return (
              <div key={stage.key} className="flex items-stretch gap-3">
                <div className="flex flex-col items-center" style={{ width: 28 }}>
                  ...
                  {!isLast && (
                    <div className="flex-1 w-px my-1 transition-all duration-300" style={{ background: lineColor, minHeight: 16 }} />
                  )}
                </div>
                <div className="flex flex-col justify-start pb-4" style={{ paddingTop: 2 }}>
                  <p
...
export function AdminVerificationPipeline({ verifications, loading, onView, onViewAll }: Props) {
  return (
    <>
      <div className="flex items-center justify-between">
        <p className="text-white font-semibold text-sm">Recent Verification Requests</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {loading ? (
              ...
              <div key={i} className="rounded-2xl p-5 space-y-4 animate-pulse"
               ...
               >
                <div className="flex justify-between">
                  <div className="h-4 w-40 rounded-md animate-pulse" style={{ background: "rgba(255,255,255,0.06)" }} />
                  <div className="h-6 w-24 rounded-full animate-pulse" style={{ background: "rgba(255,255,255,0.06)" }} />
                </div>
                <div className="flex flex-col gap-3">
                  {Array.from({ length: 5 }).map((_, j) => (
                    <div key={j} className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-full animate-pulse shrink-0" style={{ background: "rgba(255,255,255,0.06)" }} />
                      <div className="h-3 w-28 rounded-md animate-pulse" style={{ background: "rgba(255,255,255,0.06)" }} />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="col-span-3 rounded-2xl px-6 py-12 text-center" style={{ background: "#161b27", border: "1px solid rgba(255,255,255,0.06)" }}>
